<template>
    <div class="dsp">
        <template>
            <el-row :gutter="20">
                <el-col :span="8" v-for="(item,j) in divBox" :key="j" >
                                ...
                                <div class="dtjForm" @click="linkTo(item)">
                                    <el-form :model="subItem" label-width="74px" ref="subForm">
                                        ...
                                        <el-form-item :label="item.title=='pleaseSell'?'请假日期：':'离境日期：'">
                                            <span style="white-space:nowrap">{{subItem.formInput3}}</span>
                                        </el-form-item>
        ...
        linkTo(item){
            if(item.title=='goAbroad'){//出国境申请详情
                this.$http.get(this.global.serverPath4+'/process_engine/goAbroad/queryGoAbroadDetailInfoByApplicationFormId', {//传递参数
                    ...
                    this.$router.push({ path: '/dashboard3', query: { num: '1',  show1: false, show2: true, info: res.body.rows, exist: true } });//跳转到详情页面
                }, function(res) {
                    alert('请求失败！！！！！')
                });
            }else{//请销假申请详情
                this.$router.push({ path: '/LeaExamApproval', query: { alarmNo:item.subs[0].formInput1} });
            }
        },
        inquiry(){
            var self = this;
			self.$http.get(self.global.serverPath4+'/process_engine/Personal/queryAllFormsByPending', {//传递参数
                ...
                var obj1 = {}, obj2 = {}, addArr1 = [], addArr2 = [], addSubs2 = {}, addSubs1 = {}, titArr1 = [], titArr2 = [];
                    if (res.body.msg) {
                        ...
                        localStorage.setItem('dspNum',self.divBox.length);
                        // self.$emit('dspNum',self.divBox.length);//子组件传值给父组件
                    } else {
                        self.$message.warning('暂无待审批数据！');
                        localStorage.setItem('dspNum',0);
                    }
			}, function(res) {
