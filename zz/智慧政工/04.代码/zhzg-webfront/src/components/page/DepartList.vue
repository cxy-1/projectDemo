                                ...
                                <div class="departForm">
                                    ...
                                    <div class="departListTable">
                                        ...
                                        <el-table v-show="tableData.length>0?true:false" :data="tableData" style="width: 100%" >
                                            <el-table-column prop="checkType" label="核查类型" align="center"></el-table-column>
                                            <el-table-column prop="name" label="核查人员" align="center">
                                                <template slot-scope="scope">
                                                    <el-popover trigger="hover" placement="top" v-if="scope.row.name.length>25">
                                                        <div  v-if="scope.row.name.length>25" style="word-wrap:break-word;word-break:normal;width:100px;">{{scope.row.name}}</div>
                                                        <div v-if="scope.row.name.length>25" slot="reference" class="name-wrapper">
                                                            <el-tag size="medium">{{ scope.row.name.slice(0,25)}}...</el-tag>
                                                        </div>
                                                    </el-popover>
                                            ...
                                            <el-table-column prop="checkCompany" label="核查单位" align="center" ></el-table-column>
                                            <el-table-column prop="sendTime" label="核查日期" align="center"></el-table-column>
                                            <el-table-column prop="checkStatus" label="核查状态" align="center"></el-table-column>
                                            <el-table-column prop="timestamp" label="时间戳" v-if="false" align="center"></el-table-column>
                                            <el-table-column prop="idNumber" label="idCard" v-if="false" align="center"></el-table-column>
                                            <el-table-column prop="checkResult" label="核查结果 "  align="center"></el-table-column>
                                            <el-table-column label="操作" align="center">
                                                <template slot-scope="scope">
                                                    <el-button type="text" style="font-size:14px"><router-link :to="{path:'/DepartAdd',query:{alarmNo:scope.row.timestamp,checkType:scope.row.checkType,dept:scope.row.dept,clickEdit:true}}">{{scope.row.checkStatus=='未核查'?'修改':''}}</router-link></el-button>
                                                    <el-button type="text" style="font-size:14px"><router-link :to="{path:'/CheckNormal',query:{alarmNo:scope.row.timestamp}}">详情</router-link></el-button>
                                                </template>
                                            </el-table-column>
                                        </el-table>  
                                        <!--表格分页  -->
...
export default {
    ...
    methods: {
        ...
        inquiry(){//查询按钮
            ...
            if(self.valueTitle==null||self.valueTitle==''){
                self.$message.warning('请选择核查单位进行查询！');
            }else{
                const _obj={
                    ...
                    if(res.code==0){
                        ...
                        if(self.tableData.length>0){
                            for(var i=0;i<self.tableData.length;i++){
                                if(self.tableData[i].checkStatus == '未核查'||self.tableData[i].checkStatus == '核查中'){
                                    self.isDisabled1=true;//只要有未核查就不能添加核查
                                    console.log(11)
                                    return
                                }else{
                                    self.isDisabled1=false;//只要有未核查就不能添加核查
                                }
                            }
                        }
                    }else if(res.code!=1){
        ...
        init(){//初始化列表
            ...
            communication(_obj).then((res) => {
                if(res.code==0){
                    ...
                    if(self.tableData.length>0){
                        for(var i=0;i<self.tableData.length;i++){
                            if(self.tableData[i].checkStatus == '核查中'){
                                self.checking=true;
                            }else{
                                self.checking=false;
                            }
