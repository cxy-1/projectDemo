...
<template>
	<div class="portArms">
		<el-card shadow="" class="mgb20">
			...
			<div class="titIcon">
				<i class="faSelf sq-icon"></i>
				<p v-show="portArmsmId?true:false">申请单号：
					<span v-model="portArmsmId"></span>
				</p>
				<div class="btn-group">
					<el-button @click="saveForm()">驳回</el-button>
					<el-button @click="resetForm('leaveForm')">返回</el-button>
					<el-button type="primary" @click="submitLeave()">同意</el-button>
				</div>
			</div>
			<div class="steps">
				<el-steps :active="stepActive" align-center>
...
export default {
	data() {
		return {
			...
			// imgIp:'http://192.168.8.105:8083/photo/'+localStorage.getItem("loginpersonId")+".jpg",
			formInline: {
				name: '',
				alarm: '',
				unit: '',
	...
	methods: {
		...
		getTabData() {
			var that = this;
			that.$http.get(that.global.serverPath2 + '/zhzg/armLicence/queryMyArmLicenceInfoByPersonId', {
				params: {
					"personId": localStorage.getItem('loginpersonId'),
